...
**Relaunching to the device with `--terminate-existing` while a recording could still be in progress leaves orphaned junk sessions.** `AVCaptureMovieFileOutput` needs an explicit `stop()` to finalize a `.mov` (write the moov atom) — killing the process mid-recording leaves an unplayable, "media may be damaged" file, and any CSV buffers not yet flushed are lost too, sometimes with partial/duplicate-looking timestamp artifacts. Confirmed root cause for one such case in checkpoint 9's extension session. During iterative device debugging: don't relaunch while a recording might be active, or delete any session created inside that experiment window immediately afterward via the debug screen's delete feature — don't let it linger as ambiguous evidence later.
